...
function autoRoboTrimp(){if(!(0<game.global.roboTrimpCooldown)&&game.global.roboTrimpLevel){var a=parseInt(getPageSetting("AutoRoboTrimp"));0==a||game.global.world>=a&&(game.global.world-a)%5==0&&!checkIfLiquidZone()&&!game.global.useShriek&&(magnetoShriek(),MODULES.other.enableRoboTrimpSpam&&debug("Activated Robotrimp MagnetoShriek Ability @ z"+game.global.world,"graphs","*podcast"))}}
...
function PraidHarder() {
	...
	//Don't raid without being able to buy extra map levels
	if (game.global.highestLevelCleared <= 209) return
...
if (game.portal.Anticipation.level > 0) {
	...
	if (game.global.fighting && ((game.jobs.Amalgamator.owned > 0) ? Math.floor((new Date().getTime() - game.global.lastSoldierSentAt) / 1000) : Math.floor(game.global.lastBreedTime / 1000)) >= antistacklimit && (game.global.antiStacks < antistacklimit || antistacklimit == 0 && game.global.antiStacks >= 1) && (!game.global.spireActive || game.global.mapsActive))
              forceAbandonTrimps();
